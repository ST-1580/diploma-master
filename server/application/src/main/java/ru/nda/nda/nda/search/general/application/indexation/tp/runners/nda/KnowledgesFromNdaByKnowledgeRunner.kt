...
@Component
class KnowledgesFromNdaByKnowledgeRunner(
    ...
    @Value("\${nda.search.general.indexing.knowledge.by.knowledge.enabled:true}")
    override val isIndexingEnabled: Boolean,

    @Value("\${nda.search.general.indexing.knowledge.by.knowledge.reschedule.period:60}")
    override val reschedulePeriodSeconds: Long,

    @Value("\${nda.search.general.indexing.knowledge.by.knowledge.min.entity.age:60}")
    override val minEntityAge: Long,

    @Value("\${nda.search.general.indexing.knowledge.by.knowledge.max.range.seconds:600}")
    override val maxIndexingRangeSeconds: Long,

    @Value("\${nda.search.general.indexing.knowledge.by.knowledge.batch.size:30}")
    override val batchSize: Int,

    @Value("\${nda.search.general.indexing.knowledge.by.knowledge.max.batch.size:70}")
    ...
    override val runnerName: String
        get() = IndexationConstants.NdaRunner.KNOWLEDGES_FROM_KNOWLEDGE

    override fun receiveIndexingDataFromNda(request: IndexationSettingsRqDto, indexationId: Long): IndexingData {
        val knowledgesForIndex = entitiesFromNdaReceiverService.getKnowledgesByKnowledge(request).batch
    ...
    companion object {
        private val LOG = LoggerFactory.getLogger(KnowledgesFromNdaByKnowledgeRunner::class.java)
    }
}
