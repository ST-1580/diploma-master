...
@Component
class BlocksFromNdaByBlockTranslationVersionRunner(
    ...
    @Value("\${nda.search.general.indexing.block.by.block.translation.version.enabled:true}")
    override val isIndexingEnabled: Boolean,

    @Value("\${nda.search.general.indexing.block.by.block.translation.version.reschedule.period:60}")
    override val reschedulePeriodSeconds: Long,

    @Value("\${nda.search.general.indexing.block.by.block.translation.version.min.entity.age:60}")
    override val minEntityAge: Long,

    @Value("\${nda.search.general.indexing.block.by.block.translation.version.max.range.seconds:600}")
    override val maxIndexingRangeSeconds: Long,

    @Value("\${nda.search.general.indexing.block.by.block.translation.version.batch.size:30}")
    override val batchSize: Int,

    @Value("\${nda.search.general.indexing.block.by.block.translation.version.max.batch.size:70}")
    ...
    override val runnerName: String
        get() = IndexationConstants.NdaRunner.BLOCKS_FROM_BLOCK_TRANSLATION_VERSION

    override fun receiveIndexingDataFromNda(request: IndexationSettingsRqDto, indexationId: Long): IndexingData {
        val blocksForIndex = entitiesFromNdaReceiverService.getBlocksByBlockTranslationVersion(request).batch
    ...
    companion object {
        val LOG = LoggerFactory.getLogger(BlocksFromNdaByBlockTranslationVersionRunner::class.java)
    }
}
